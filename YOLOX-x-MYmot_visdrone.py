...
class Exp(MyExp):
    def __init__(self):
        ...
        self.nmsthre = 0.7
        self.no_aug_epochs = 3  #最后几轮没有数据增强，设置为0就是使用数据增强
    ...
    def get_model(self, settings=None):
        ...
        if getattr(self, "model", None) is None:
            ...
            if settings is not None:
                moco = MoCo(backbone, head, settings['seq_names'], dim=head.emb_dim)    # 创建了一个MoCo模型实例，将YOLOX的主干网络和头部网络与MoCo模型结合，利用MoCo进行对比学习
            else:
                moco = None
            self.model = YOLOX(backbone, head, moco=moco)                 #MoCo 的训练是在每次训练迭代中与 YOLOX 模型一起进行的，对比学习在整个训练过程中起到了关键作用。
                                                                            #MoCo模型能够辅助YOLOX模型在训练过程中获得更好的特征表示
