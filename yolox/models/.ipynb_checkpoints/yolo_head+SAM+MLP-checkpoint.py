...
from .network_blocks import BaseConv, DWConv, Upsample, AutomaticWeightedLoss, AdaptiveAvgPoolMLP, SpatialAttention
...
class YOLOXHead(nn.Module):
    def __init__(
        ...
        self.reid_preds = nn.Sequential(
                            ...
                            AdaptiveAvgPoolMLP(
                                input_channels=int(256 * width) * 3,  # 输入通道数，与前一层输出匹配
                                output_size=768,             # 输出维度，通常设为嵌入维度
                                hidden_size=256                       # 隐藏层大小，可以根据需要调整
                            ),
                            # 2 BaseConv layers
                            Conv(
                                in_channels=int(256 * width) * 3,
