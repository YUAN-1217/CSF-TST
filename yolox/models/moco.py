...
class MoCo(object):
    ...
    def __init__(self,
        ...
        self.encoder_k_head = deepcopy(head)                        #是 YOLOX 的 backbone 和 head 的深度拷贝，形成了 key encoder（即 momentum encoder）
    ...
    @torch.no_grad()
    def _momentum_update_key_encoder(self, backbone, head):           #通过动量机制逐步更新 key encoder 的参数，使其捕捉到历史帧的特征
        """                                                         
        Momentum update of the key encoder
        """                                                            #逐步更新 key encoder，使其保留历史帧的特征，而 query encoder 则更新为当前帧的特征。
        for param_q, param_k in zip(backbone.parameters(),
                                    self.encoder_k_backbone.parameters()):
    ...
    def _gen_pos_logits(self, feat_q, feat_k, id_q=None, id_k=None):        #用于生成正样本对，通过当前帧的 ReID 特征 feat_q 和前一帧的 ReID 特征 feat_k 进行匹配
        if id_q is None or (id_q == -1).all(): 
            ...
            valid = values >= self.sim_thresh                               #若匹配到的对象属于同一类别并且相似度高于阈值 self.sim_thresh，则认为是正样本
    ...
    def _gen_neg_logits_intra(self, feat_q, feat_k):                    #当前帧内的不同对象之间生成负样本，self._gen_neg_logits_intra() 用于计算相似度并生成负样本。
        similarity = torch.einsum('nc,mc->nm', feat_q, feat_k)
        diag = torch.diag_embed(torch.diag(similarity) - 1e18)
        similarity = similarity + diag
        return similarity

    def _gen_neg_logits_inter(self, feat_q, seq_list_q, cls_q, intra_num=0):        #不同帧的对象生成负样本,这部分负样本从历史队列（queue）中取出，队列中存储了不同帧的特征
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue(self, k_feats_dict):           #通过将历史帧的特征存入队列，并不断更新，保持负样本的多样性
    ...
    def _gen_logits_with_ids(self,          #生成对比学习中的正负样本对，并通过伪标签的匹配来生成正负对比的logits
                             seq_list,      #生成伪标签的过程也是为了确保在不同帧之间正确匹配目标，以便在对比学习时能进行正确的正负样本对比
                             q_reid,
                             k_reid,
                             q_ids,
                             k_ids,
        ...
        if len(q_reid) * len(k_reid) == 0:
            ...
            # positive logits: Nx1
            valid_q, valid_k, l_pos = self._gen_pos_logits(q_reid, k_reid, q_ids, k_ids)#生成正样本对，代码是通过比较当前帧（query）和历史帧（key）的ReID特征 (q_reid 和 k_reid) 
                                                                                        #以及它们的ID (q_ids 和 k_ids)，找到匹配的正样本对（positive pairs），即同一个目标在不同帧的特征
            ...
            if len(valid_q) == 0:
                ...
                # negative logits: NxK              #生成负样本对（negative pairs）
                # negative samples within current frame
                l_neg_intra = self._gen_neg_logits_intra(q_reid[valid_q], q_reid[valid_q])#同帧内的负样本 (l_neg_intra)：即当前帧中不属于同一目标的不同物体的特征
                # negative samples within adjacent two frames
                l_neg_intra_2 = self._gen_neg_logits_intra(q_reid[valid_q], k_reid[valid_k])#相邻帧间的负样本 (l_neg_intra_2)：相邻两帧间不属于同一目标的特征对
                # inter-class variance is contained
                intra_num = len(l_neg_intra) + len(l_neg_intra_2)
                # negative samples in other videos
                l_neg_inter = self._gen_neg_logits_inter(q_reid[valid_q],   #其他视频或序列中的负样本 (l_neg_inter)：从其他视频或序列中选择不同的物体作为负样本
                                                         seq_list,
                                                         q_cls[valid_q],
                                                         intra_num=intra_num)
                # total logits: Nx(1+K)
