...
# Copyright (c) Alibaba, Inc. and its affiliates.加了一些TensorBoard可视化的东西
...
from yolox.tracker.CFS_TST_tracker import CFS_TST_Tracker, DefaultArgs
...
class Trainer:
    ...
    def train_one_iter(self):
        ...
        #if self.rank == 0: #TensorBoard 记录学习率的变化，帮助你监控学习率调度策略在训练过程中的表现
            #self.tblogger.add_scalar('lr', lr, self.epoch * self.max_iter + (self.iter + 1))
    ...
    def before_train(self):
        ...
        if self.rank == 0:
            ...
            #在 TensorBoard 中看到整个模型的架构图，帮助你理解模型结构
            dummy_input = torch.randn(1, 3, *self.input_size).to(self.device)  # 创建一个模拟输入
            self.tblogger.add_graph(self.model, dummy_input)  # 可视化模型结构
        ...
        self.infer_loader = self.exp.get_infer_loader(
            batch_size=self.args.batch_size,
            is_distributed=self.is_distributed)
        #self.infer_pseudo_sync()                                                       #在这里载入各种训练数据
    ...
    def after_epoch(self, end=False):
        ...
        # 保存每一轮的权重文件
        ckpt_name = f"epoch_{self.epoch + 1}"
    ...
    def save_ckpt(self, ckpt_name, update_best_ckpt=False):
        if self.rank == 0:
            ...
            logger.info(f"Save weights to {self.file_name} with ckpt_name: {ckpt_name}")#"Save weights to {}".format(self.file_name)
            ckpt_state = {
