#!/usr/bin/env python3
# -*- encoding:utf-8 -*-
# Copyright (c) Alibaba, Inc. and its affiliates.#修改基础版-添加一些东西来做对比实验  track里还添加了相应参数
##T2C（Tentative-to-Confirmed Association Strategy ）


###又加了小修改是对于update中的predict的内容和gmc的内容
# 预测轨迹位置加入tentivate和lost，之后应用GMC运动补偿到所有轨迹  


#最后的输出部分改成只输出已确认轨迹了记得修改
...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, cls=0, feat=None, feat_history=50):
        ...
        self.alpha = 0.9
##########################################################################
        self.birth_count = 0 # 新增:记录连续检测次数

        self.birth_threshold = None  # 新增:轨迹确认所需帧数
##########################################################################
...
#################################################################
    def activate(self, kalman_filter, frame_id):
        ...
        if frame_id == 1:  # 第一帧特殊处理
            ...
            if self.birth_threshold <= 0:  # 如果阈值设为1或更小，直接确认
                self.state = TrackState.Tracked
                self.is_activated = True
            else:
                self.state = TrackState.Tentative  # 否则设为待确认状态
                self.is_activated = False
...
# ##########################################################################
#         # self.birth_count = 1
#         # self.state = TrackState.Tentative # 初始为待确认状态

#         self.birth_count = 1
#         if self.birth_threshold == 1:  # 特殊情况处理
#             self.state = TrackState.Tracked
#         else:
...
##########################################################################
        ...
        #     self.state = TrackState.Tracked # 连续2帧检测则确认
        #     self.birth_count = 0
...
class DefaultArgs(object):
    def __init__(self, mot20=False):
        ...
        self.birth_threshold = 1  # 新增确认阈值参数
...
######################################################################################
        ...
        self.max_time_lost = self.buffer_size    #定义了跟踪目标消失多少帧会被标记为完全丢失
...
#################################################################
    ...
    def update(self, output_results, img_info, img_size, embeddings=None, img=None):
        ...
        # 预测轨迹位置
        # STrack.multi_predict(confirmed_tracks)
        # STrack.multi_predict(tentative_tracks)
        ...
        #     warp = self.gmc.apply(img, bboxes[high_mask] if len(output_results) else None)
        #     STrack.multi_gmc(confirmed_tracks, warp)
        #     STrack.multi_gmc(tentative_tracks, warp)
