#!/usr/bin/env python3
# -*- encoding:utf-8 -*-
# Copyright (c) Alibaba, Inc. and its affiliates.只跟踪单类别
...
#其是对整个文件夹进行检测而不是单独的文件夹列如在video文件夹中包含所有帧那么就需要在往前一个文件夹try/video这样运行
#python tools/track.py my_video_sequence --benchmark VisDrone -f exps/example/u2mot/yolox_x_u2mot_visdrone.py -c pretrained/u2mot/visdrone.pth.tar --device 0 --fp16 --fuse --filter-class 0
def make_parser():
    ...
    # 添加类别过滤参数:0代表值跟踪第一类即人，1只跟踪第二类即车，-1代表跟踪所有类别
    parser.add_argument("--filter-class", type=str, default=-1, help="only track specific classes, comma separated integers (e.g., '0,3,4'), or -1 for all classes")
    # 添加birth threshold参数
    parser.add_argument("--birth_threshold", type=int, default=3, help="frames needed for track confirmation")
    return parser
...
def image_track(predictor, vis_folder, res_folder, video_path, args):
    ...
    for frame_id, (img, img_info) in enumerate(dataloader, start+1):
        ...
        if outputs[0] is not None:
            ...
            online_scores = []
            online_cls = []
            # 在函数开始处解析filter_class参数为列表
            filter_classes = []
            if args.filter_class != "-1":
                filter_classes = [int(cls_id) for cls_id in args.filter_class.split(",")]
            ...
            for t in online_targets:
                ...
                # 添加类别过滤条件
                # 修改类别过滤条件，支持多类别
                if filter_classes and int(t.cls) not in filter_classes:
                    continue                                                  #111
